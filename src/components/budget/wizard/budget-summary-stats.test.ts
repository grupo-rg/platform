...
describe('computeBudgetStats', () => {
    ...
    it('counts partidas resolved across multiple events', () => {
        const subs: SubEvent[] = [
            _ev({ id: '1-item_resolved', kind: 'resolved', title: 'C01.01 Demolición', detail: '✓ €120' }),
            _ev({ id: '2-item_resolved', kind: 'resolved', title: 'C01.02 Otra', detail: '✓ €450' }),
            _ev({ id: '3-item_resolved', kind: 'resolved', title: 'C02.01 Albañilería', detail: '✓ €1.200' }),
        ];
        const s = computeBudgetStats(subs);
        expect(s.partidasCount).toBe(3);
    });

    it('does NOT count phase-milestone resolved events (inline_fast_path_used, tabular_parser_completed)', () => {
        const subs: SubEvent[] = [
            _ev({ id: '1-item_resolved', kind: 'resolved', title: 'C01.01 X', detail: '✓ 100 €' }),
            _ev({ id: '2-item_resolved', kind: 'resolved', title: 'C01.02 Y', detail: '✓ 200 €' }),
            // Estos dos eventos tienen kind='resolved' pero NO son partidas — son
            // hitos visuales del timeline. No deben sumar al partidasCount/pemTotal.
            _ev({ id: 'A-inline_fast_path_used', kind: 'resolved', title: '15 partidas extraídas' }),
            _ev({ id: 'B-tabular_parser_completed', kind: 'resolved', title: 'Parser TABULAR · 15 partidas' }),
        ];
        const s = computeBudgetStats(subs);
        expect(s.partidasCount).toBe(2);
        expect(s.pemTotal).toBeCloseTo(300, 2);
    });

    it('extracts chapter prefix from resolved title and counts distinct', () => {
        const subs: SubEvent[] = [
            _ev({ id: '1-item_resolved', kind: 'resolved', title: 'C01.01 X', detail: '✓ €1' }),
            _ev({ id: '2-item_resolved', kind: 'resolved', title: 'C01.02 Y', detail: '✓ €2' }),
            _ev({ id: '3-item_resolved', kind: 'resolved', title: 'C02.01 Z', detail: '✓ €3' }),
            _ev({ id: '4-item_resolved', kind: 'resolved', title: 'C04.05 W', detail: '✓ €4' }),
        ];
        const s = computeBudgetStats(subs);
    ...
    it('aggregates pemTotal from price detail strings (es-ES locale)', () => {
        const subs: SubEvent[] = [
            _ev({ id: '1-item_resolved', kind: 'resolved', title: 'A', detail: '✓ 1.200,50 €' }),
            _ev({ id: '2-item_resolved', kind: 'resolved', title: 'B', detail: '✓ 350 €' }),
            _ev({ id: '3-item_resolved', kind: 'resolved', title: 'C', detail: '✓ 50.000,00 €' }),
        ];
        const s = computeBudgetStats(subs);
    ...
    it('counts anomalies from kind=error events', () => {
        const subs: SubEvent[] = [
            _ev({ id: '1-item_resolved', kind: 'resolved', title: 'A', detail: '✓ €100' }),
    ...
    it('formats pemTotal with es-ES currency formatter', () => {
        const subs: SubEvent[] = [
            _ev({ id: '1-item_resolved', kind: 'resolved', title: 'A', detail: '✓ 12.345,67 €' }),
        ];
        const s = computeBudgetStats(subs);
    ...
    it('parses es-ES thousands without decimals correctly (regression)', () => {
        ...
        const subs: SubEvent[] = [
            _ev({ id: '1-item_resolved', kind: 'resolved', title: 'A', detail: '✓ 1.500 €' }),
            _ev({ id: '2-item_resolved', kind: 'resolved', title: 'B', detail: '✓ 12.345 €' }),
            _ev({ id: '3-item_resolved', kind: 'resolved', title: 'C', detail: '✓ 95.980 €' }),
        ];
        const s = computeBudgetStats(subs);
    ...
    it('handles missing detail gracefully', () => {
        const subs: SubEvent[] = [
            _ev({ id: '1-item_resolved', kind: 'resolved', title: 'C01.01 Sin precio' }),
            // El Intl formatter es-ES emite "100 €" (número primero, € al final).
            _ev({ id: '2-item_resolved', kind: 'resolved', title: 'C01.02 X', detail: '✓ 100 €' }),
        ];
        const s = computeBudgetStats(subs);
        expect(s.partidasCount).toBe(2);
